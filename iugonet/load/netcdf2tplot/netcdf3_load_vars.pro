...
;         
; Purpose:
;         Returns a structure with the data from general netCDF files
;         Based on netcdf_load_vars.pro
;         
; Input:
...
function netcdf3_load_vars, ncfile
  for fileindex = 0, n_elements(ncfile)-1 do begin
    ...
    if size(netCDFi, /type) ne 8 then begin ; check that 'netCDFi' structure doesn't already exist
        ...
        ; store number of dimensions in the netCDFi structure
        str_element, netCDFi, 'ndim', ndims, /add_rep

        ; loop through and store the dimension name
        for i = 0, ndims-1 do begin
          ncdf_diminq, file, i, name, size
          if i eq 0 then begin
              dims = create_struct(name, size)
          endif else begin
              str_element, dims, name, size, /add_rep
          endelse
          str_element, netCDFi, 'dims', dims, /add_rep
        endfor
