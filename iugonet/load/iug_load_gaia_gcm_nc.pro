...
;
;NAME:
; iug_load_gaia_gcm_nc
;
;PURPOSE:
;  Queries the NICT servers for the GAIA model (gcm part) data
;  provided by the GAIA project and loads data into tplot format.
;
;SYNTAX:
; iug_load_gaia_gcm_nc, parameter = parameter, downloadonly = downloadonly, trange = trange, verbose = verbose, uname=uname, passwd=passwd
;
;KEYWOARDS:
;  parameter = first parameter name of GAIA gcm data.  
;          For example, iug_load_gaia_gcm_nc, parameter = 'gao'.
...
pro iug_load_gaia_gcm_nc, parameter = parameter, downloadonly = downloadonly, trange = trange, verbose = verbose, uname = uname, passwd = passwd
  ...
  ;--- all parameters (default)
   parameter_all = strsplit('gao gmo gu gv gw gt gz gvw gr gp',' ', /extract)
  ...
  ;**************************
   ...
   for j=0L,n_elements(parameters)-1 do begin
      ...
      if ~size(fns,/type) then begin
        ...
        ;****************************
         file_names = file_dailynames(file_format = 'YYYY/' + parameters[j] + 'YYYYMMDD',trange = trange,times = times, /unique)+'gcm.nc'
        ...
        ;===============================
         ...
         source.local_data_dir = root_data_dir() + 'gaia/wk3/gaia/'+parameters[j]+'_gcm/'
        ;source.local_data_dir = '/mnt/data7/gaia/'+parameters[j]+'_gcm/'
         source.remote_data_dir = 'https://aer-nc-web.nict.go.jp/gaia/wk3/gaia/'+parameters[j]+'_gcm/'
     ...
     ;----Load the GAIA data in case of downloadonly = 0----
      if (downloadonly eq 0) then begin
        ...
        ;---Initialize data and time buffer
         ...
         for k=ks,n_elements(local_paths)-1 do begin
           ...
           ;---Check wheter the specified file exists or not---
            if file_test(/regular,file) then  dprint,'Loading gaia data file: ',file $
            else begin
               dprint,'gaia data file ',file,' not found. Skipping'
               continue
            endelse
           ...
           ;---Get the variable:
            ncdf_varget, cdfid, 'time', time     ;0.50 to 23.50 hrs
            ncdf_varget, cdfid, 'lat', latitude     ;87.86 to -87.86 degs
            ncdf_varget, cdfid, 'lon', longitude     ;0 to 360 degs
            ncdf_varget, cdfid, 'lvl', pressure     ;997.5 to 1.017 ×10^-9 hPa
            ncdf_varget, cdfid, parameters[j], data
            
            data_size = size(data) 
            if data_size(0) eq 4 then begin

              ;---Definition of arrary names:
               gaia_data = fltarr(n_elements(time),n_elements(longitude),n_elements(latitude),n_elements(pressure))
              ...
              ;---Definition of arrary names:
               gaia_data = fltarr(n_elements(time),n_elements(longitude),n_elements(latitude))
        ...
        ;---Acknowlegment string (use for creating tplot vars)
         ...
         if (size(gaia_data,/type) eq 4) or (size(gaia_data,/type) eq 3) then begin
           ;---Create tplot variables and options for GAIA data:
            ...
            if data_size(0) eq 4 then begin
               store_data, 'gaia_gcm_'+parameters[j], data = {x:unix_time_app, y:gaia_data_app, glat:latitude, glon:longitude, press:pressure},dlimit=dlimit
               options, 'gaia_gcm_' + parameters[j], ztitle = data_long_name + ' [' + data_units +']'
            endif else begin
               store_data, 'gaia_gcm_'+parameters[j], data = {x:unix_time_app, y:gaia_data_app, glat:latitude, glon:longitude},dlimit=dlimit              
            endelse
         endif
      endif
      new_vars=tnames('gaia_gcm_'+parameters[j])
